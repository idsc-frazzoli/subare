...
class Sarsa_Dynamaze {
  static void handle(SarsaType sarsaType, int nstep, int EPISODES) throws Exception {
    ...
    LearningRate learningRate = DefaultLearningRate.of(5, 1.11);
    Sarsa sarsa = sarsaType.supply(dynamaze, qsa, learningRate);
    GifSequenceWriter gsw = GifSequenceWriter.of(UserHome.Pictures("dynamaze_qsa_" + sarsaType + ".gif"), 200);
    for (int index = 0; index < EPISODES; ++index) {
      ...
      DiscreteVs vs = DiscreteUtils.createVs(dynamaze, qsa);
  ...
  public static void main(String[] args) throws Exception {
    ...
    // handle(SarsaType.expected, 3, 50);
